...
def enthalpyDiff(self,iii):
    ...
    if np.any(delta_mass_liq<0):
        if np.any(np.abs(delta_mass_liq[delta_mass_liq<0])>1e-7):
            ...
            print(self.modeltime[iii],iii,'Uh oh. Fixing negative values of delta_mass_liq, min value:',min(delta_mass_liq))
            iGL = np.where(delta_mass_liq==min(delta_mass_liq))[0]
            print('iGL:',iGL)
            print('z',self.z[iGL])
            print('LWC mass',(self.LWC[iGL]*RHO_W_KGM))
            print('mass_liq',mass_liq[iGL])
            print('count:', count)
